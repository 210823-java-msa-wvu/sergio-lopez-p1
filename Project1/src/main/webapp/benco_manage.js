let baseUrl = "http://localhost:8083/Project1/benco";

function viewAllExceed(){
    ...
    //open the request
    xhr.open("put","http://localhost:8083/Project1/benco/exceed");
    //sent the request
    xhr.send();
...
function viewAll(){
    ...
    xhr.onreadystatechange = receiveData;
    //open the request
    xhr.open("get","http://localhost:8083/Project1/benco");
    //sent the request
    xhr.send();
...
function changeAmount(){
    ...
    let amount = document.getElementById("new_amount").value;
    ...
    //open the request
    xhr.open("put",baseUrl+`/change/${reimburse_id}`);
    xhr.setRequestHeader("Content-Type", "application/json");
    //sent the request
    xhr.send(`{"amount":"${amount}","reason":"${reason}"}`);

    function callback(){
        if(xhr.readyState==4&&xhr.status==403){
            let warn = document.getElementById("warning");
            warn.innerHTML='<div class="alert alert-danger"><strong>Not Allowed!</strong>You are not allowed to change amount for that reimbursement.</div>';
        }
...
function confirm(){
    ...
    xhr.onreadystatechange = callback;
    //open the request
    xhr.open("put",baseUrl+`/confirm_grade/${reimburse_id}`);
    //sent the request
    xhr.send();
    ...
    function callback(){
        if(xhr.readyState==4&&xhr.status==403){
            let warn = document.getElementById("warning");
            warn.innerHTML='<div class="alert alert-danger"><strong>Not Allowed!</strong>You are not allowed to confirm grade for that reimbursement.</div>';
        }
        else if(xhr.readyState==4&&xhr.status==400){
            let warn = document.getElementById("warning");
            warn.innerHTML='<div class="alert alert-danger"><strong>Bad Request!</strong>Unable to find the reimbursement</div>';
...
function loadManager(){
    ...
    xhr.onreadystatechange = receiveData;
    //open the request
    xhr.open("get","http://localhost:8083/Project1/manager_manage");
    //sent the request
    xhr.send();
...
function redirectBack(){
    window.open("http://localhost:8083/Project1/manager_manage.html","_self");
}
