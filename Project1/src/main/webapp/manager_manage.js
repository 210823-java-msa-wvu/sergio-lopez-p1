let baseUrl = "http://localhost:8083/Project1/manager_manage";

function bencoRedirect(){
    let m = JSON.parse(JSON.parse(getCookie("managerinfo")));
    if(m.rank==3){
        window.open("http://localhost:8083/Project1/benco_manage.html","_self");
    }
    else{
        let warn = document.getElementById("warning");
        warn.innerHTML='<div class="alert alert-warning"><strong>Not Allowed!</strong>This is BENCO only feature.</div>';
    }
}
...
function uploadInfo(){
    ...
    //open the request
    xhr.open("put",baseUrl+`/upload/${reimburse_id}`);
    //sent the request
    xhr.send();
...
function declineReimbursement(){
    ...
    xhr.onreadystatechange = callback;
    //open the request
    xhr.open("put",baseUrl+`/decline/${reimburse_id}`);
    //sent the request
    xhr.send(reason);
    ...
    function callback(){
        if(xhr.readyState==4&&xhr.status==403){
            let warn = document.getElementById("warning");
            warn.innerHTML='<div class="alert alert-danger"><strong>Not Allowed!</strong>You are not allowed to decline that reimbursement.</div>';
        }
        else if(xhr.readyState==4&&xhr.status==400){
            let warn = document.getElementById("warning");
            warn.innerHTML='<div class="alert alert-danger"><strong>Bad Request!</strong>Unable to find the reimbursement</div>';
...
function signReimbursement(){
    ...
    xhr.onreadystatechange = callback;
    //open the request
    xhr.open("put",baseUrl+`/sign/${reimburse_id}`);
    //sent the request
    xhr.send();
    ...
    function callback(){
        if(xhr.readyState==4&&xhr.status==403){
            let warn = document.getElementById("warning");
            warn.innerHTML='<div class="alert alert-danger"><strong>Not Allowed!</strong>You are not allowed to sign that reimbursement.</div>';
        }
        else if(xhr.readyState==4&&xhr.status==400){
            let warn = document.getElementById("warning");
            warn.innerHTML='<div class="alert alert-danger"><strong>Bad Request!</strong>Unable to find the reimbursement</div>';
...
function confirm(){
    ...
    xhr.onreadystatechange = callback;
    //open the request
    xhr.open("put",baseUrl+`/confirm_presentation/${reimburse_id}`);
    //sent the request
    xhr.send();
    ...
    function callback(){
        if(xhr.readyState==4&&xhr.status==403){
            let warn = document.getElementById("warning");
            warn.innerHTML='<div class="alert alert-danger"><strong>Not Allowed!</strong>You are not allowed to confirm presentation for that reimbursement.</div>';
        }
        else if(xhr.readyState==4&&xhr.status==400){
            let warn = document.getElementById("warning");
            warn.innerHTML='<div class="alert alert-danger"><strong>Bad Request!</strong>Unable to find the reimbursement</div>';
...
function requestInfoM(){
    ...
    xhr.onreadystatechange = callback;
    //open the request
    xhr.open("put",baseUrl+`/requestinfo_m/${reimburse_id}`);
    //sent the request
    xhr.send(rank);
    ...
    function callback(){
        if(xhr.readyState==4&&xhr.status==403){
            let warn = document.getElementById("warning");
            warn.innerHTML='<div class="alert alert-danger"><strong>Not Allowed!</strong>You are not allowed to request info from that manager.</div>';
        }
        else if(xhr.readyState==4&&xhr.status==400){
            let warn = document.getElementById("warning");
            warn.innerHTML='<div class="alert alert-danger"><strong>Bad Request!</strong>Unable to find the reimbursement</div>';
...
function requestInfoE(){
    ...
    xhr.onreadystatechange = callback;
    //open the request
    xhr.open("put",baseUrl+`/requestinfo_e/${reimburse_id}`);
    //sent the request
    xhr.send();
    ...
    function callback(){
        if(xhr.readyState==4&&xhr.status==403){
            let warn = document.getElementById("warning");
            warn.innerHTML='<div class="alert alert-danger"><strong>Not Allowed!</strong>You are not allowed to request info on that reimbursement.</div>';
        }
        else if(xhr.readyState==4&&xhr.status==400){
            let warn = document.getElementById("warning");
            warn.innerHTML='<div class="alert alert-danger"><strong>Bad Request!</strong>Unable to find the reimbursement</div>';
...
function populateData(resp){
    ...
    head.innerHTML = "<th>Id</th><th>Applicant</th><th>amount</th><th>Grade Method</th><th>Pass grade</th><th>Grade</th><th>Direct Manager</th><th>Depart Head</th><th>BENCO</th><th>Location</th>"+
    "<th>Start Date</th><th>Submit Date</th><th>Last Sign Date</th><th>Status</th><th>Note</th>";
    ...
    for(let i=0;i<resp.length;++i){
        ...
        entry.innerHTML += `<td>${resp[i].note}</td>`;
...
function redirectBack(){
    window.open("http://localhost:8083/Project1/manager.html","_self");
}
